...
def get_delay(uid):
    return delay_cfg.get(uid, 1.0)

F1, F2 = "bot_users.txt", "banned_users.txt"
if os.path.exists(F2):
    with open(F2, "r") as f:
        b_u = set(int(l.strip()) for l in f if l.strip())
else:
    b_u = set()
...
def _logic(c, u_i):
    def _mk(cid):
        w_m[f"{u_i}_{cid}"] = datetime.datetime.now(datetime.timezone.utc)

    async def _sd(cid, ct, tid=None):
        s_t[u_i] = True
        inf = isinstance(ct, str)
        ls = [ct] if inf else ct
        n = 0
        while s_t.get(u_i):
            for m in ls:
                if not s_t.get(u_i):
                ...
                try:
                    ...
                    await asyncio.sleep(get_delay(u_i))
                    n += 1
                    if n % 10 == 0:
    ...
    async def _inf(e):
        ...
        try:
            if target:
                if target.isdigit():
                    user = await c.get_entity(int(target))
                else:
                    user = await c.get_entity(target)
            elif e.is_reply:
            ...
            await e.edit("❌ **Không tìm thấy người này!**")
    ...
    async def _fk(e):
        ...
        try:
            if t:
                target = await c.get_entity(int(t) if t.isdigit() else t)
            elif e.is_reply:
                target = await c.get_entity((await e.get_reply_message()).sender_id)
            else:
                return await e.edit("⚠️ Tag @, ID hoặc Reply!")
        except:
            return await e.edit("❌ Không thấy!")
        ...
        try:
            ...
            if p:
                ...
                if curr_p:
                    await c(functions.photos.DeletePhotosRequest(id=[types.InputPhoto(id=ph.id, access_hash=ph.access_hash, file_reference=ph.file_reference) for ph in curr_p]))
            await e.edit("✅ Xong"); await asyncio.sleep(1); await e.delete()
        except:
            await e.edit("❌ Lỗi Fake")

    @c.on(events.NewMessage(outgoing=True, pattern=r'/diefake'))
    async def _dfk(e):
        if u_i not in o_p:
            return await e.edit("⚠️ Chưa lưu gốc!")
    ...
    async def _sp1(e):
        ...
        if os.path.exists('chui.txt'):
            await _sd(e.chat_id, open('chui.txt', 'r', encoding='utf-8').readlines(), t)
    ...
    async def _sp2(e):
        ...
        if os.path.exists('spam2.txt'):
            await _sd(
                e.chat_id,
                open('spam2.txt', 'r', encoding='utf-8').readlines(),
                t
            )
    ...
    async def _spi(e):
        ...
        try:
            cnt = int(e.pattern_match.group(1))
        except:
            cnt = 10
    ...
    async def _spn(e):
        ...
        while s_t.get(u_i):
            try:
                await c.send_message(e.chat_id, v);await asyncio.sleep(get_delay(u_i))
    ...
    async def _setdelay(e):
        value = float(e.pattern_match.group(1))
        if value < 0.0001 or value > 5.0:
            return await e.edit("❌ Chỉ từ 0.0001 -> 5.0 giây")
        delay_cfg[u_i] = value
        await e.edit(f"✅ Delay = {value}s")
    ...
    async def _cl2(e):
        ...
        if not st:
            await e.edit("⚠️ No data"); await asyncio.sleep(1); await e.delete(); return
    ...
    async def _cam1(e):
        ...
        if not u and e.is_reply:
            u = str((await e.get_reply_message()).sender_id)
        if not b:
            b = str(e.chat_id)
        if u:
            k = f"{u_i}_{b}_{u}"
    ...
    async def _cam2(e):
        ...
        if not u:
            u = str(e.chat_id) if e.is_private else (str((await e.get_reply_message()).sender_id) if e.is_reply else None)
        if u:
            k = f"{u_i}_{u}"
            if m == "camib":
                c_i[k] = True
    ...
    async def _lo(e):
        ...
        try:
            u_c.pop(u_i, None)
            await c.log_out()
            if os.path.exists(f"u_{u_i}.session"):
                os.remove(f"u_{u_i}.session")
            await e.delete()
        except:
            pass
    ...
    async def _br(ev):
        ...
        if a_r.get(u_i) and ev.sender_id != u_i:
            try:
                await c(functions.messages.SendReactionRequest(peer=ev.chat_id, msg_id=ev.id, reaction=[types.ReactionEmoji(emoticon='❤️')]))
            except:
                pass
        if o_f.get(u_i) and ev.is_private and ev.sender_id != u_i:
            try:
                await ev.reply("đây là tin nhắn tự động, Tao đang bận không thấy off à nhắn cc")
            except:
                pass
...
async def _ubn(e):
    ...
    if u in b_u:
        ...
        await e.respond("⚠️ ID này không nằm trong danh sách ban!")
...
async def _tb(e):
    ...
    if not os.path.exists(F1):
        return await e.respond("⚠️ Chưa có người dùng nào trong danh sách!")
